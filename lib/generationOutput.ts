...
export function normalizeGenerationOutput(value: unknown, sourceUrl = ''): GenerationOutput {
  ...
  return {
    ...
    target_audience: text(item.target_audience, '일반 시청자'),
    video_goal: text(item.video_goal, '콘텐츠 목적 미지정'),
    ...
    structure_analysis: {
      ...
      audience_problem: text(structure.audience_problem, '시청자의 관심사 또는 궁금증'),
      ...
      product_placement_pattern: text(structure.product_placement_pattern, '핵심 대상이 자연스럽게 등장하는 흐름'),
