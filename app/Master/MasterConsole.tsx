...
interface AuditRow {
  id: string; admin_user_id?: string; user_id?: string; actor_user_id?: string | null; actor?: { full_name?: string | null; email?: string } | null; action: string; target_type: string; target_id: string | null;
...
function actionLabel(action: string): string {
  return ({
    ...
    'trend.update': '피드 수정', 'trend.delete': '피드 삭제', 'trend.restore': '피드 복원', 'trend.delete_bulk': '피드 일괄 삭제', 'trend.restore_bulk': '피드 일괄 복원', 'trend.purge': '피드 영구 삭제', 'trend.purge_bulk': '피드 일괄 영구 삭제', 'trend.refresh': '트렌드 피드 새로고침', 'profile.update': '사용자 프로필 수정', 'inquiry.status_update': '문의 상태 변경',
...
function AuditsPanel({ data, kind, setKind, page }: { data: PageData<AuditRow>; kind: 'admin' | 'user'; setKind: (kind: 'admin' | 'user') => void; page: (v: number) => void }) { return <div><div className="mb-4 flex gap-2"><button type="button" aria-pressed={kind === 'admin'} onClick={() => setKind('admin')} className={`rounded-xl px-3 py-2 text-xs font-bold ${kind === 'admin' ? 'bg-violet-600' : 'bg-white/5 text-white/45'}`}>관리자 로그</button><button type="button" aria-pressed={kind === 'user'} onClick={() => setKind('user')} className={`rounded-xl px-3 py-2 text-xs font-bold ${kind === 'user' ? 'bg-violet-600' : 'bg-white/5 text-white/45'}`}>사용자 로그</button></div><div className="overflow-x-auto rounded-2xl border border-white/8"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-white/[.035] text-xs text-white/40"><tr><th className="p-4">작업</th><th>대상</th><th>변경 내용</th><th>{kind === 'user' ? '사용자' : '관리자'}</th><th className="pr-4">일시</th></tr></thead><tbody className="divide-y divide-white/6">{data.audits?.map((row) => <tr key={row.id}><td className="p-4 font-bold">{actionLabel(row.action)}</td><td className="text-white/50">{row.target_type} · {row.target_id?.slice(0, 8) ?? '-'}</td><td className="max-w-xs truncate text-white/50">{row.reason ?? (row.before_data?.full_name !== row.after_data?.full_name ? `${String(row.before_data?.full_name ?? '이름 없음')} → ${String(row.after_data?.full_name ?? '이름 없음')}` : '-')}</td><td className="text-xs text-white/45">{row.actor?.full_name || row.actor?.email || (kind === 'admin' && !row.actor_user_id ? '시스템' : row.actor_user_id?.slice(0, 8) ?? '-')}</td><td className="pr-4 text-xs text-white/40">{formatDate(row.created_at)}</td></tr>)}</tbody></table>{!data.audits?.length && <Empty />}</div><Pagination page={data.page} total={data.total} size={data.pageSize} go={page} /></div>; }
