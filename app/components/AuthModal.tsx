    ...
    try {
      ...
      if (mode === 'signup') {
        ...
        if (error) {
          ...
          const emailAlreadyExists = normalizedMessage.includes('already registered')
            || normalizedMessage.includes('already exists')
            || normalizedMessage.includes('email exists')
            || error.code === 'user_already_exists';
          setMessage({ type: 'error', text: emailAlreadyExists ? t('auth_email_already_exists') : msg || t('auth_network_error') });
          setLoading(false);
          return;
        }
              ...
              {mode === 'suspended' ? (
                ...
                <form onSubmit={handleFindEmail} className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <PhoneCountrySelect value={phoneCountryCode} countryIso={phoneCountryIso} onChange={(dial, iso) => { setPhoneCountryCode(dial); setPhoneCountryIso(iso); }} />
